...
- 重建 .app bundle → 跑 `bash build_app.sh`（生成 `~/Applications/WhisperPro.app`，解決 macOS TCC 權限歸帳問題）
...
- **預設**：`right_cmd`（單按右 Cmd ⌘）— 適合右側只有 Cmd / Option 的緊湊鍵盤
- **點按切換（tap toggle）**：按下放開＝開始錄音；再按下放開＝停止錄音並自動轉錄。每次「完整按下→放開」算 1 個 tap，fire 在 release 上（按住 5 秒再放開仍只算 1 次，不會 start+stop）。與螢幕上的錄音按鈕行為一致。
- `config.py` 讀取時會自動把舊的 `cmd+shift+space` 重置為 `cmd+alt+r`（沿用）；舊 `cmd+alt+r` 設定**不會**被自動遷移到 `right_cmd`，使用者要自己改

### 支援的 hotkey 字串格式

**Lone modifier（單一鍵，新）** — `parse_hotkey()` 回單一鍵且該鍵為側別 modifier：
- `right_cmd` / `left_cmd`
- `right_option` / `left_option`（亦接受 `right_alt` / `left_alt`）
- `right_ctrl` / `left_ctrl`
- `right_shift` / `left_shift`
- pynput 對應：`Key.cmd_r` / `Key.alt_l` / `Key.ctrl_r` / `Key.shift_l` / ...
- 注意 pynput 在 macOS 上 `Key.alt == Key.alt_l`、`Key.cmd == Key.cmd_l`（同值），所以 `left_*` 在 pynput 層次與「未指定側別」的同名鍵無法區分；`right_*` 則明確區分（這也是預設挑右 Cmd 的原因）

**Combo（組合鍵，既有）** — `+` 分隔，例如：`cmd+alt+r` / `ctrl+shift+space`，邏輯完全不動

### Dead-key 保護（lone-modifier 專屬）

Right Option 在 macOS 上會輸入特殊字元（Option+e=é）。`HotkeyManager._on_p_lone` / `_on_r_lone` 靠 `_other_key_during_press` 旗標保護：armed 期間若有任何其他鍵被按下，release 時就不 fire。另外按下目標 modifier 之前若 `_pressed` 不為空，本次 press 不 armed。所以「Right Option + 字母」的正常打字流程不會誤觸 hotkey。

### HotkeyBindDialog（重新綁定對話框）

仍用 Tk-native binding（macOS 26.4+ TSM 主執行緒限制，pynput 在此禁用）。`_keysym_to_name` 改成側別感知（meta_r → `right_cmd`），偵測規則：max_combo 恰好 1 個側別 modifier → 鎖定為 lone；含至少 1 個非修飾鍵 → 鎖定為 combo（sided modifier 自動 normalize 成 `cmd`/`alt`/...）；其他（只有多個 modifier 而無字母）→ 提示需加字母。
...
17. **左右側 modifier 同按、放開一顆會被誤判**（Fix 9 P1-A / 2026-05-22）— pynput / NSEvent 的 `event.modifierFlags()` 對 `cmd_l` / `cmd_r`（以及 alt/ctrl/shift 左右）共用同一個 device-independent bit（如 `NSEventModifierFlagCommand`）。情境「按 Left Cmd → 按 Right Cmd → 放開 Left Cmd」放開時 modifierFlags 仍含 cmd bit（因 cmd_r 還按著），舊邏輯誤判 release 為 press → `_pressed` 累積 stale `Key.cmd_l`，lone-mode 下次乾淨 press 無法 arm 需等 10s self-heal。**判斷 FlagsChanged 是 press 還是 release 不能只看 modifier bit**，必須用 set 自己做 state machine：sided keycode 在集合中 = release，否則 = press。實作於 `hotkey_manager.py` `_handle_ns_event` FlagsChanged 分支用 `self._ns_held_modifiers: set[int]` 追蹤；`stop()` 同步 clear。詳見 `docs/superpowers/plans/2026-05-22-pr-13-code-review-fixes.md` §2 P1-A。
...
7. **積極使用 sub-agent**（2026-05-21 新規）— 只要工作能拆成獨立子任務、或需要橫跨多檔案探索/編輯，**優先 dispatch sub-agent 並行執行**，不要在主對話裡硬幹。範例：
   - Bug 修復：一個 bug 一個 agent，多個 bug 並發 dispatch
   - 多檔案探索：用 `Explore` 或 `general-purpose` agent
   - 測試 / regression：交給 agent 跑，主對話保留協調
   - **只有「meta 規則修改」或「單行 5 秒內完成的編輯」直接在主對話做**
   原則：能拆就拆、能並發就並發。主對話扮演「協調者 + 驗證者」，agent 做實際的編輯。
   驗證鐵律：agent 回報完工後**必定**回讀檔案確認改對地方，不要只信 agent 的 summary。
8. **每次更新都要改版本號與更新日期**（2026-09-25 使用者指示）— 任何要合進 main 的改動，同一個 commit 裡都要：
   ① 改 `_version.py` 的 `__version__`（修 bug +0.0.1、新功能 +0.1.0）
   ② 在同檔的 `_RELEASE_DATES` 加一行 `"新版號": "當天日期"`（設定 →「關於」會顯示在版本號底下）
   漏填日期 `tests/test_version.py` 會紅燈；**但漏改版本號沒有任何測試擋得住，只能靠這條規則**——
   v2.31.0、v2.31.1 就是 commit 標題寫了新版號、`_version.py` 卻沒改，使用者在「關於」看到 2.30.0 以為沒更新到。
