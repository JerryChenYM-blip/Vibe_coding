...
> 這份檔案讓任何新的 Codex session 能無縫接手這個專案。
> 位置：`/Users/jerrychen/project/Codex/AGENTS.md`
...
### 執行 App
```bash
cd /Users/jerrychen/project/Codex
...
2. **所有文件檔案一律用繁體中文撰寫**（2026-04-19 新規）— 包含：
   - 所有 `.md` 文件（AGENTS.md、使用手冊、策略文件、README 等）
...
4. **不要主動建立 `.md` 檔** — 除非使用者明確要求（這份 AGENTS.md 是被明確要求的例外）。
...
**AGENTS.md 結束** — 任何新 session 讀完這份就能直接接手。如果專案有大改動，**請同步更新這份檔案**。
