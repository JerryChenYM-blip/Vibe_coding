...
def _i_history(p: _Pen) -> None:
    ...
    構圖：3/4 圓弧（12→3→6→9 點方向）+ 頂部開口 + 左上指向箭頭，
    圓心放一個短時針表達「時間」概念。
    """
    # 以 4 段折線模擬 3/4 圓弧（從 12 點上方略偏左開始，順時針到 9 點下方）
    # 改用近似八邊形外圈，視覺上跟 Lucide 的 history 圖示一致。
    # 上半圓開口（12 點位置左側有個缺口）
    p.polyline([
        (10, 4), (14, 4), (18, 7), (20, 12),
        (18, 17), (14, 20), (10, 20), (6, 17), (5, 13),
    ])
    # 逆時針箭頭（從圓弧左上方外伸，類似 refresh 符號）
    p.polyline([(5, 13), (2, 13), (5, 10)])
    # 中央時針：從圓心指向 2 點方向（代表「時間」）
    p.line(12, 12, 15, 9)
    # 圓心圓點
    p.dot(12, 12, r=1.2)
