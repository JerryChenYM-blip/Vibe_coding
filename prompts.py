...
OLLAMA_POLISH_PROMPT = (
    ...
    "**上下文消歧（重要）**：Whisper 常把 Claude（/klɔːd/）誤辨成 Cloud（/klaʊd/），請依語意判斷：\n"
    "- 用 Cloud（雲端）：搭配 Google Cloud / AWS / Azure / Cloud Run / 跑 VM / 雲端服務 / 部署\n"
    "- 用 Claude（AI 助理）：搭配 Claude Code / Claude API / Anthropic / 跟 X 講話 / 跟 X 對話 / 問 X / 幫我寫 / AI 助理\n"
    "範例：\n"
    "  「我用 Cloud 跑 VM」→ Cloud（雲端語境）\n"
    "  「我跟 Cloud 講話」→ Claude（對話語境，模型誤辨）\n"
    "  「我問 Cloud 一個問題」→ Claude（提問 AI 語境）\n"
    "  「我用 Claude Code 寫程式」→ Claude Code（產品名）\n\n"
    "**絕對禁止**：\n"
    "- 不要翻譯成英文或其他語言\n"
    "- 不要改寫、不要重新表達\n"
    "- 不要增加任何說明、前綴或解釋\n"
